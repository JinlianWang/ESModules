...
- `src/layout.module.css` supplies the shared layout styles. The host (`src/index.js`) imports the module and applies its hashed class names to the `<body>`, heading, and each section so CSS stays scoped without leaking global selectors.
...
- `npm run build` runs Vite in library mode, cleans `dist/` via a pre-build plugin, and minifies both the library bundle and the copied demo `.js` files (plus CSS Modules) by default. Passing `--minify false` disables minification everywhere so you can debug readable builds.
...
- When adding styles, prefer CSS Modules (see `src/layout.module.css`) and apply the hashed class names through JavaScript so bare HTML stays framework-agnostic.
